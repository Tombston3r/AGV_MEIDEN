...
agv::esp32::EspModemPower g_modem_power(agv::board::kModemPwrkey, agv::board::kWatchdogDone);
...
extern "C" void app_main() {
  ...
  g_spi.begin(SPI2_HOST, agv::board::kSpiSclk, agv::board::kSpiMosi, agv::board::kSpiMiso,
              agv::board::kSpiCs, CFG_BUS_SPI_FREQ_HZ);
  ...
  if (store->read("aes_nonce", nonce_blob, sizeof(nonce_blob)) == sizeof(nonce_blob)) {
    cellular.channel().set_nonce_counter((static_cast<uint32_t>(nonce_blob[0]) << 24) |
                                     (static_cast<uint32_t>(nonce_blob[1]) << 16) |
                                     (static_cast<uint32_t>(nonce_blob[2]) << 8) |
                                     nonce_blob[3]);
  }

  static agv::AgvApp app(profile, g_clock, cellular, sequencer, queue, &g_indicators);
  g_rt.app = &app;
