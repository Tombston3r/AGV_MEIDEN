...
constexpr gpio_num_t kPinNss = GPIO_NUM_5;   // relevé : U2.NSS -> U1 pad 29
// ⚠ RELEVÉ SUR LA V6.0 : la broche RESET du RFM95W n'est PAS câblée. Le pilote
// reçoit donc une broche factice, et `begin()` ne peut pas forcer un reset
// matériel — un module figé ne se récupère qu'en coupant l'alimentation carte.
// À corriger sur une révision ultérieure : une GPIO libre suffit.
constexpr uint8_t kPinReset = 0xFF;   // non câblée sur la V6.0
constexpr uint8_t kPinDio0 = 26;      // IO26, relevé au KiCad
