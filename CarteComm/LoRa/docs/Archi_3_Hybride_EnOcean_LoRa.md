...
**Chaîne de réception.** Le TCM 515 reçoit le télégramme et le remonte à l'ESP32 en UART à 57600 bauds au format **ESP3** (EnOcean Serial Protocol 3) : trame `0x55` + en-tête + CRC8 en-tête + données + CRC8 données. L'ESP32 parse, extrait l'ID 32 bits et le code de touche (le PTM 210 a 4 contacts : A0, A1, B0, B1).
