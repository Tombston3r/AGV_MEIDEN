# État du projet — carte de communication AGV MEIDEN
...
>
> Dernière mise à jour : **2026-08-13** — création du monorepo complet.
...
| Indicateur | Valeur |
|---|---|
| Tests natifs | **122 tests, 441 assertions, 0 échec** |
...
| Lignes de code (hors docs) | ~11 700 |
...
### 1.5 Transports (§5) — trois implémentations derrière `ITransport`

| Transport | État | Particularités implémentées |
|---|---|---|
| `LoraTransport` | ✅ | Ordonnanceur **half-duplex** explicite, fenêtre d'ACK, 3 retransmissions, **budget de rapport cyclique 1 %/1 h qui refuse réellement l'émission** et remonte le refus en défaut visible |
...
| `MqttLteTransport` | ✅ | SIM7080G, topics `agv/<id>/{cmd,ack,telemetry,status}`, QoS 1, **Last Will and Testament** |
...
- `PosteApp` : chaîne EnOcean → appairage → LoRa, instantané de supervision,
...
| `test_transport.cpp` | 16 | Budget légal, half-duplex, pile AT, modem muet |
| `test_app.cpp` | 16 | Persistance NVS, chien de garde, `agvdump` |
| `test_poste.cpp` | 13 | Chaîne EnOcean → LoRa complète |
...
| Relevé de couverture LoRa le long du parcours | Choix du SF (voir `latence_lora.md`) | `lora.spreading_factor` |
...
La même clé doit être installée sur l'AGV, le poste et **chaque bouton LoRa**.
...
./tools/flash.sh bouton       /dev/ttyUSB2    # un flash par bouton
...
Pour chaque bouton LoRa, modifier avant flash `kNodeId` et `kStation` dans
`firmware/bouton-lora/src/main.cpp` — ce sont les **seules** lignes à changer,
et aucune modification n'est nécessaire côté AGV.
...
| Commandes refusées | `tx_refused_duty` → budget légal épuisé ; voir `latence_lora.md` |
| Appuis sans effet | `enocean_unpaired` → bouton non appairé |
| Liaison qui tombe | `rssi_dbm`, `rx_bad_crc`, `telemetry_age_ms` |
...
| B8 | Choisir le SF LoRa | Relevé de couverture + arbitrage latence (`latence_lora.md`) |
...
| T3 | Implémenter `radio_begin` / `radio_send` / `radio_wait_ack` du bouton LoRa | M | Déclarés `extern`, à câbler sur `Sx1276Radio` |
...
| S4 | SF adaptatif LoRa (SF7 nominal, repli SF9) | Seulement si le relevé montre des zones difficiles |
...
| `LoraTransport` | Half-duplex, retransmissions, budget ERC 70-03 effectif |
...
| 122 tests natifs | Dont un par ligne du §12 |
