...
- **LoRa** : parcours complet avec relevé RSSI/SNR à chaque point d'arrêt,
  machines en marche. Vérifier que le budget de rapport cyclique reste sous
  100 % en exploitation nominale (`duty_used_permille` dans `/agvdump`).
- **Cellulaire** (seulement si l'architecture 2 est envisagée) : RSRP/RSRQ en
  tous points. Un seul point sous −110 dBm disqualifie la solution
  (Archi_2 §7.1). Puis 200 aller-retours réels, avec distribution complète —
  c'est le 99ᵉ percentile qui compte, pas la moyenne.
