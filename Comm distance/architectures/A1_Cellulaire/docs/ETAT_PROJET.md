...
> dossier d'architecture ; extraction des sources LoRa vers `architectures/A2_LoRa/`.
>
...
L'implémentation LoRa vit dans le dossier d'architecture `architectures/A2_LoRa/`.
...
| T11 | Faire de `architectures/A2_LoRa/` un dossier d'architecture autonome | M | Aujourd'hui simples sources extraites ; recette dans son README |
...
| 2026-08-13 | Réorganisation en un dossier autonome par architecture. Le projet entier passe sous `architectures/A1_Cellulaire/` ; les sources LoRa (transport, budget ERC 70-03, SX1276, bouton sur pile, 10 tests) sont extraites vers `architectures/A2_LoRa/`. Les deux `main.cpp` passent sur le transport cellulaire. | Périmètre, compteurs (122 → 112 tests), transports, kanban (T3 retiré, T11/T12 ajoutés), déploiement (plus de flash de bouton) |
