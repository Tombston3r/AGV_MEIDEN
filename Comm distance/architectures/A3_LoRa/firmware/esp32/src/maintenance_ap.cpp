...
bool AccessPoint::start_wifi() {
  // La pile Wi-Fi est déjà initialisée par le mode station : on ne fait
  // qu'ajouter l'interface AP.
  wifi_config_t cfg = {};
  ...
  // APSTA et non AP : la liaison cliente vers le réseau d'entreprise doit
  // survivre à l'ouverture de la fenêtre de maintenance.
...
void AccessPoint::stop_wifi() {
  ...
  // On retire le point d'accès SANS arrêter la pile : le mode station reste
  // actif, l'AGV ne perd pas sa liaison au poste fixe.
