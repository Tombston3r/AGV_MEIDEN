...
>
> **Périmètre** : `architectures/A3_Wifi/`, projet autonome et zippable. La carte AIO
...
|---|---|---|
| 2026-08-28 | **Fusion des deux variantes LoRa et boutons rendus optionnels.** Elles ne se distinguaient que par la technologie du bouton : les boutons étant devenus optionnels, cette distinction ne définissait plus une architecture. `A2_Hybride` et `A3_LoRa` deviennent **`A2_LoRa`**, l'ancienne `A4_Wifi` devient **`A3_Wifi`** pour garder une numérotation continue. **Les boutons sortent des nomenclatures** : le planning seul fait rouler l'AGV, et leur choix reste ouvert, indépendant de l'architecture. Disparaissent avec eux le point de bascule entre variantes et les tableaux « coût par station », qui ne mesuraient que des boutons. Le poste ESP32 de l'ancienne A2 est supprimé (le poste est Linux), ainsi que le décodeur ESP3 en C++ de ce dossier, celui du poste étant en Python dans le banc. **Le banc EnOcean est conservé** : il validera les `PTM 210` si l'option est retenue. Trois architectures, un seul critère de choix : la liaison du poste vers l'AGV. A2 = 417 €, A3 = 592 €, A1 = 426 € plus 96 €/an. | arborescence, BOM, comparatif |
| 2026-08-28 | **Modèle d'architecture commun** : boutons d'appel optionnels, **poste central** hébergeant l'API de planning et recevant les appels, carte AGV Control, AGV. Les quatre architectures ne diffèrent plus que par la technologie de chaque liaison ([`ARCHITECTURE_COMMUNE.md`](../../../../docs/ARCHITECTURE_COMMUNE.md)). Trois conséquences. **A3 gagne un poste** (UniPi Lite 1.1, +209 € HT) et **perd son argument principal**, l'absence d'infrastructure ; elle conserve le bouton authentifié et l'accusé visuel. **Les postes ESP32 de A1 et A2 passent sous Linux** : héberger une API web, un planning persistant et un journal sur un microcontrôleur coûte trop en logiciel. **Le Timer quitte la carte AGV** pour le poste, ce qui fait disparaître le DS3231, la contrainte NVS et le serveur web à rendre permanent. Les quatre solutions se tiennent désormais entre **525 et 700 € HT** : le coût cesse d'être discriminant. ⚠️ **A2 et A3 ne sont plus séparées que par 19 € et le type de bouton** : question ouverte au client dans `COMPARAISON.md`. | les 4 architectures, BOM, comparatif |
...
| 2026-08-26 | **Banc LoRa : l'écran et le bouton de la T-Beam sont exploités.** Le profil de carte ne suffisait pas, sans écran, le relevé de portée exige encore de promener un ordinateur au bout d'un câble USB. `oled.{h,cpp}` est un pilote SSD1306 minimal (texte seul, aucune dépendance tierce) qui affiche RSSI, SNR, compteurs et tension batterie ; sous −115 dBm il annonce `!! MARGE FAIBLE`. Le bouton de la carte déclenche **une** émission à la demande, pour mesurer au point où l'on se trouve plutôt qu'au rythme d'une boucle. La police 5×7 est **vérifiée par rendu** (`test_hote/rendu_police.cpp`) : un `static_assert` et le rendu ASCII ont pris un antislash en fin de commentaire qui avalait le glyphe suivant, 94 glyphes au lieu de 95. Avec deux T-Beam, le relevé se fait à deux personnes, sans matériel informatique. | banc LoRa |
| 2026-08-26 | **Banc LoRa : prise en charge de la LILYGO T-Beam 868 MHz.** Elle porte le **même SX1276** que le `RFM95W` de la V6.0 (le `RFM95W` n'est qu'un module HopeRF enveloppant cette puce) donc le pilote du projet fonctionne tel quel. `test_config.h` devient un profil de carte choisi par drapeau (`CARTE_V6` / `CARTE_TBEAM`), quatre environnements PlatformIO. ⚠️ **Sur T-Beam la radio est alimentée par un AXP192**, pas directement : sans activation de LDO2 le SX1276 est hors tension et `RegVersion` lit `0x00`, ce qui fait chercher un défaut de câblage SPI inexistant. `src/tbeam_power.cpp` s'en charge, coupe le GPS et remonte un diagnostic lisible selon la révision (v0.7 sans PMU, v1.2 en AXP2101). Deux avantages au banc : batterie intégrée pour le relevé de portée sans ordinateur, et broche `RESET` câblée contrairement à la V6.0. **Verdict pour la télécommande finale : non en production** (4 à 14 mois d'autonomie sur 18650 contre 5 à 8 ans pour le bouton A3 sur `ER14505`) **mais oui comme prototype**, pour éprouver le protocole avant d'engager un PCB. | banc LoRa |
| 2026-08-26 | **Réorganisation du dépôt par NATURE d'objet.** `architectures/` (les 4 solutions), `bancs/` (les essais matériels), `materiel/` (les projets KiCad), `docs/` (brief et comparatif), `outils/` (scripts). Le dossier `CarteComm/` disparaît. **Le matériel n'est plus dupliqué** : une carte, un projet, un seul endroit, trois copies divergentes de la V6.0 avaient déjà coexisté, dont une que KiCad avait recréée à un chemin renommé, et il a fallu comparer les schémas pour trouver la vivante (celle du 26 août, avec l'embase J3 et les Gerbers). Le **brief** passe de 4 copies à 1. Les **bancs LoRa sortent de `A2_LoRa/test/`** pour rejoindre `bancs/lora/`, à la même forme que le banc EnOcean ; `test/native/` ne garde que les tests unitaires. Chaque banc a désormais son `README.md` **et** son `DEPLOY.md` avec une recette et les résultats attendus. `outils/exporter_architecture.sh` reconstitue une architecture autonome dans un zip, la zippabilité est payée à l'export, plus tous les jours. Le générateur de nomenclatures n'a plus de chemin absolu. **2 Mo de redondance supprimés.** | arborescence, tous les renvois |
...
| 2026-08-25 | **A2 et A3 permutés** : l'hybride EnOcean + LoRa devient **A2**, le LoRa pur devient **A3**, l'hybride a été étudié avant. Les dossiers suivent : `A3_Hybride/` → `A2_LoRa/` et `A2_LoRa/` → `A2_LoRa/`, ainsi que les documents de référence. Chaque dossier ne garde désormais **que sa propre spécification** : la copie étrangère héritée de la duplication est retirée. Le brief et l'index racine sont réordonnés pour se lire A1, A2, A3, A3. **Les variables du générateur cessent d'encoder un numéro d'architecture**, `bouton_a1` devient `bouton_pile`, `poste_a3` devient `poste_enocean`, `a1_ht` devient `ht_lora_pur` : c'est ce qui faisait revenir ce travail à chaque permutation. `subd25_atmega.md` reste exclu, `A2`/`A3` y désignant des broches de connecteur. | arborescence, BOM, brief, README |
| 2026-08-25 | **Réorganisation : un dossier par ARCHITECTURE et non plus par mode de communication.** `SMS_EnOcean/` → `A1_Cellulaire/`, `Wifi/` → `A3_Wifi/`, `LoRa/` → `A2_LoRa/`, et **`A2_LoRa/` est créé**. A3 et A2 étaient jusqu'ici un dossier de sources non autonome : ils reçoivent le cœur métier de A3 (c'est lui qui porte le duo MEGA + ESP32 de la carte V6.0, pas le firmware mono-ESP32 de A1) plus le transport LoRa, et A2 y ajoute le décodage ESP3 et le poste fixe. **Les quatre dossiers compilent et testent seuls** : 112, 119, 130 et 109 tests. Côté matériel, `AIO_AGV_Control_V6.0` quitte A3 pour A3 et A2 : le relevé KiCad montre que c'est la V5.0.1 **plus un `RFM95W-868S2`** (58 empreintes contre 57), câblé `NSS`→`IO5`, `SCK`→`IO18`, `MISO`→`IO19`, `MOSI`→`IO23`, `DIO0`→`IO26`, exactement le brochage supposé. ⚠️ **`RESET` n'est pas câblée**, `kPinReset` passe à `0xFF` et une carte de kanban est ouverte. ⚠️ Dette assumée : le firmware ESP32 de A3 et A2 porte encore la passerelle MQTT héritée de A3, à remplacer par un `LoraGatewayApp` (kanban LG1). | arborescence, BOM, README, journaux |
| 2026-08-25 | **A1 et A3 permutés** : le cellulaire devient **A1**, le LoRa homogène devient **A3**, la numérotation suit désormais l'ordre chronologique d'étude, le SMS ayant précédé le LoRa. A2 (hybride) et A3 (Wi-Fi) sont inchangés. Permutation appliquée à 10 fichiers, dont **le brief**, dont la table des architectures est aussi réordonnée ; les deux copies restent identiques. Les documents de référence sont renommés (`Archi_1_Cellulaire_SMS_LTE-M.md` et `Archi_2a_LoRa_P2P_homogene.md`) et leurs six renvois suivis. **Deux faux positifs protégés** : le repère KiCad `Mega2560 Pro (A1)`, et l'intégralité de `subd25_atmega.md`, où `A1`/`A3` désignent des broches de connecteur et non des architectures. | brief, BOM, README, COMPARAISON, DEPLOY |
| 2026-08-25 | **Numérotation A1–A3 rendue explicite dans les nomenclatures.** Le retrait des accessoires couvrait déjà les quatre architectures (42 lignes), mais seules A3 et A2 portaient leur code : les titres, les sections et les blocs Total nomment désormais **A1** (cellulaire) et **A3** (Wi-Fi) au même titre. `README.md` gagne une colonne `Code` et `COMPARAISON.md` complète ses libellés. `tools/prix_a_completer.md` est aligné : 7 lignes d'accessoires retirées, section « Boîtiers » supprimée, section 2 renommée « Radio », et une **enveloppe de sourcing** (~1 630 € HT) ajoutée, explicitement présentée comme une borne d'effort de recherche et **non un budget**, puisqu'elle additionne des alternatives qui s'excluent. | BOM, README, COMPARAISON, feuille de sourcing |
