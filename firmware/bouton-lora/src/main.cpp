...
void setup() {
  ...
  if (!radio_begin(profile.lora, kLoraCs, kLoraReset, kLoraDio0)) {
    blink_failure();
    sleep_forever();
  }
  ...
  bool acked = false;
  for (uint32_t attempt = 0; attempt < profile.lora.max_tries && !acked; ++attempt) {
    ...
    acked = radio_wait_ack(f.seq, profile.lora.ack_timeout_ms, channel);
  }
