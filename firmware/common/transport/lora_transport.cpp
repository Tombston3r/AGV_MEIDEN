...
bool LoraTransport::begin() {
  if (!radio_.begin(profile_.lora)) return false;
  radio_.listen();
...
bool LoraTransport::send(const Frame& f) {
  ...
  pending_airtime_us_ = lora_airtime_us(tx_len_, profile_.lora.spreading_factor,
                                        profile_.lora.bandwidth_hz, profile_.lora.coding_rate);
...
void LoraTransport::tick() {
  ...
  switch (tx_state_) {
    ...
    case TxState::AwaitAck:
      if ((now_ms - last_tx_ms_) >= profile_.lora.ack_timeout_ms) {
        if (tries_ >= profile_.lora.max_tries) {
          ++health_.tx_failed;
