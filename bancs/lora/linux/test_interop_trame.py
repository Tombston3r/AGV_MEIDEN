...
# Ce contrôle compare le portage Python au codec C++ DU PROJET : la dépendance
# vers une architecture est donc voulue, c'est tout l'intérêt de l'essai. On
# prend la première qui porte le codec, elles sont identiques sur ce point.
def _codec_cpp() -> pathlib.Path:
    for archi in sorted((RACINE / "architectures").glob("A*")):
        commun = archi / "firmware" / "common"
        if (commun / "proto" / "frame.cpp").is_file():
            return commun
    raise SystemExit(
        "codec C++ introuvable : ce contrôle a besoin d'un dossier "
        "architectures/A*/firmware/common/proto/. Lancé hors du dépôt ?"
    )
