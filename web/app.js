...
function render(data) {
  ...
  const duty = data.duty_permille ?? 0;
  $('duty').textContent = `${(duty / 10).toFixed(1)} % du budget`;
  $('duty').classList.toggle('alarm', duty >= 900);
...
async function post(path, payload, feedbackId) {
  ...
  try {
    ...
    feedback.textContent = data.ok
      ? 'Commande transmise.'
      : 'Commande REFUSÉE par le poste (transport occupé ou budget d’émission épuisé).';
